...
public class Exercise25_01 {
    ...
    public class BinaryTree<E extends Comparable<E>> extends AbstractTree<E> {
        ...
        //helper method
        public int height(TreeNode catRoot){
            if (catRoot == null) {
                return 0;
            }
            else {
                //Jason showed me this after he saw what I'd made, and I asked if it could put on one line.
//                return Math.max(height(catRoot.left), height(catRoot.right)) +1;
                int catDepthLeft = height(catRoot.left);
                int catDepthRight = height(catRoot.right);
                if (catDepthLeft > catDepthRight){
                    return catDepthLeft + 1;
                }
                else {
                    return catDepthRight + 1;
                }
            }
        }
        ...
        public void breadthFirstTraversal() {
            ...
            queue.clear();
            queue.add(root); //Adding root to list
            while(!queue.isEmpty()){
                TreeNode<E> current = queue.element();
                //Adding the left element if not empty

                if(current.left != null){
                    queue.add(current.left);
                }
                //Adding the right element if not empty
                if(current.right != null){
                    queue.add(current.right);
                }
                //Printing the left and right element and removing it form the list
